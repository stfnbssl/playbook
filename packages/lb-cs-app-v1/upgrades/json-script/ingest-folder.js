 ...
 *
 * Usage:
 *   node ingest-folder.js --dir ./samples --base-url http://localhost:3001 --concurrency 3
 *
 * Options:
 *   --dir           directory to scan for .json files (required)
 *   --base-url      base URL of JSON Insight API (default: http://localhost:3001)
 *   --pattern       substring that matched in filename (default: .json)
 *   --recursive     include subfolders (boolean flag)
 *   --merge         MERGE all json files into a single ingestion (array) instead of one dataset per file (boolean flag)
 *   --concurrency   number of parallel ingestions (default: 2)
 *   --dry-run       do not POST, only show what would be sent (boolean flag)
 *   --timeout       request timeout ms (default: 45000)
 *
 * Env:
 *   API_KEY         sent as 'x-api-key' header if present (backend may enforce ADMIN_KEY)
 */
...
async function walk(dir, recursive) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      if (recursive) {
        files.push(...await walk(p, recursive));
      }
    } else {
      files.push(p);
    }
