...
// indexes for cache collection
LLMCacheSchema.index({ "stats.createdAt": -1 });
LLMCacheSchema.index({ provider: 1, model: 1 });
try {
  // optional text index on result content
  LLMCacheSchema.index({ "result.content": "text" } as any);
} catch { /* some mongo versions disallow multiple text indexes at runtime */ }
