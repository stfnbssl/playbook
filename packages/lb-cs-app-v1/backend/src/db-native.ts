...
export async function connectNativeMongo(uri?: string): Promise<Db> {
  if (_db) return _db;
  ...
  await json_records().createIndex({ datasetId: 1, chunkIndex: 1 });
  const ttl = process.env.DATASET_TTL_SECONDS ? Number(process.env.DATASET_TTL_SECONDS) : 0;
  if (ttl > 0) {
    try { await json_datasets().createIndex({ createdAt: 1 }, { expireAfterSeconds: ttl }); } catch {}
  }
  return _db;
}

export function json_datasets(): Collection<DatasetDoc> {
  if (!_db) throw new Error("DB non inizializzato");
  return _db.collection<DatasetDoc>("datasets");
}
export function json_records(): Collection<RecordDoc> {
  if (!_db) throw new Error("DB non inizializzato");
  return _db.collection<RecordDoc>("records");
}
export function json_views(): Collection<ViewDoc> {
  if (!_db) throw new Error("DB non inizializzato");
