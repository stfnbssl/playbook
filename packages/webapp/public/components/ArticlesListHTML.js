...
// Componente ArticlesListHTML - Specializzato per HTML con ancore
const ArticlesListHTML = ({ 
    articles = [], 
    onArticleSelect, 
    currentArticle = null,
    ...
    useEffect(() => {
        const filtered = articles.filter(article =>
            ...
            (article.anchor && article.anchor.toLowerCase().includes(searchTerm.toLowerCase()))
        );
    ...
    const handleArticleClick = (articleNumber) => {
        if (onArticleSelect) {
    ...
    const handleMouseEnter = (e, isActive) => {
        if (!isActive) {
            e.target.closest('.article-item').style.background = '#f8f9fa';
        } else {
            e.target.closest('.article-item').style.background = '#2980b9';
        }
    };
    ...
    const handleMouseLeave = (e, isActive) => {
        if (!isActive) {
            ...
            e.target.closest('.article-item').style.background = '#3498db';
        }
    };
                ...
                <ul style={styles.articlesList}>
                    {filteredArticles.length === 0 ? (
                        ...
                        filteredArticles.map((article) => {
                            const isActive = currentArticle === article.number;
                            return (
                                <li
                                    key={article.number}
                                ...
                                >
                                    <div style={styles.articleLink}>
                                        ...
                                        <div style={{
                                            ...styles.anchorInfo,
                                            ...(isActive ? styles.anchorInfoActive : {})
                                        }}>
                                            🔗 {article.anchor || `#art_${article.number}`}
                                        </div>
                                    </div>
                                </li>
                            );
...
window.ArticlesListHTML = ArticlesListHTML;